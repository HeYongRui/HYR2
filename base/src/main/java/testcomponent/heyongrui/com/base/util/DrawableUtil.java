...
public class DrawableUtil {
    ...
    public static Drawable tintDrawable(Context context, @DrawableRes int drawableResId, int color) {
        ...
        ColorStateList colors = ColorStateList.valueOf(ContextCompat.getColor(context, color));
     ...
     * 对目标Drawable 进行着色
     */
    public static Drawable tintDrawable(Context context, @NonNull Drawable drawable, int color) {
        Drawable wrappedDrawable = getCanTintDrawable(drawable);
        ColorStateList colors = ColorStateList.valueOf(ContextCompat.getColor(context, color));
        DrawableCompat.setTintList(wrappedDrawable, colors);
        return wrappedDrawable;
    ...
    public static class DrawableBuilder {
        ...
        private int mGradientShapeType;//形状类型
        ...
        public GradientDrawable createGradientDrawable() {
            ...
            if (mGradientColors != null && mGradientColors.length > 0) {//渐变色
                for (int i = 0; i < mGradientColors.length; i++) {
                    mGradientColors[i] = ContextCompat.getColor(mContext, mGradientColors[i]);
                }
                gradientDrawable = new GradientDrawable(mGradientOrientation, mGradientColors);
            } else {//填充色
                gradientDrawable = new GradientDrawable();
                if (mGradientSolidColors == null && mColor != 0) {
                    mGradientSolidColors = ColorStateList.valueOf(ContextCompat.getColor(mContext, mColor));
                }
            ...
            if (mGradientStrokeColor == null && mColor != 0) {
                mGradientStrokeColor = ColorStateList.valueOf(ContextCompat.getColor(mContext, mColor));
            }
        ...
        public ShapeDrawable createRingDrawable() {
            ...
            if (mColor != 0) {
                circularRingDrawable.getPaint().setColor(ContextCompat.getColor(mContext, mColor));
            }
            if (mRingShader != null) {
